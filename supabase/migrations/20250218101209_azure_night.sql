/*
  ...
  1. Changes
    - Ensures required extensions are installed
    - Creates auth schema with proper settings
    - Sets up auth.users table with all required columns
...
CREATE UNIQUE INDEX IF NOT EXISTS users_email_idx ON auth.users (email) WHERE email IS NOT NULL AND deleted_at IS NULL;
